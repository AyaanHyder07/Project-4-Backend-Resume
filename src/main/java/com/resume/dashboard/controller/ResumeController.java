 ...
 * RESUME CONTROLLER
 *
 * Owner endpoints (auth required, userId injected from JWT filter): POST
 * /api/resumes → create resume (plan-gated: limit + template) GET
 * /api/resumes/{resumeId} → get own resume by id PATCH
 * /api/resumes/{resumeId}/meta → update title / profession PATCH
 * /api/resumes/{resumeId}/theme → change theme POST
 * /api/resumes/{resumeId}/submit → submit for admin approval POST
 * /api/resumes/{resumeId}/publish → publish (plan-gated: public link limit)
 * POST /api/resumes/{resumeId}/unpublish → unpublish DELETE
 * /api/resumes/{resumeId} → delete
 *
 * Public endpoint (no auth): GET /api/public/resumes/{slug} → view public
 * published resume by slug
 *
 * Admin endpoint: POST /api/admin/resumes/{resumeId}/approve → approve a
 * submitted resume
 */
@RestController
public class ResumeController {
	...
	/*
	 * ================================================================ CREATE
	 * RESUME Plan checks inside service: resume limit + template gating
	 * ================================================================
	 */
	@PostMapping("/api/resumes")
	public ResponseEntity<Resume> create(@AuthenticationPrincipal String userId,
	...
	public ResponseEntity<Resume> updateMeta(@AuthenticationPrincipal String userId, @PathVariable String resumeId,
		...
		return ResponseEntity
				.ok(resumeService.updateMeta(userId, resumeId, request.getTitle(), request.getProfessionType()));
	}

	/*
	 * ================================================================ CHANGE THEME
	 * No plan check needed here — changing to a different base theme is allowed on
	 * all plans. Theme customization (colors/fonts) is gated separately in
	 * UserThemeCustomizationController.
	 * ================================================================
	 */
	...
	public ResponseEntity<Resume> submitForApproval(@AuthenticationPrincipal String userId,
	...
	/*
	 * ================================================================ PUBLISH
	 * RESUME Plan checks inside service: subscription active + public link limit
	 * ================================================================
	 */
	...
	/*
	 * ================================================================ ADMIN —
	 * APPROVE RESUME Approval is an admin-only action, handled in
	 * AdminResumeController. Kept here as a stub comment for clarity — implement in
	 * admin layer. ================================================================
	 */
